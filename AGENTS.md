...
- Frontend tests (`npx vitest run`) include Storybook browser-mode tests that need Playwright Chromium; the update script runs `npx playwright install chromium`.
...
- **`DEMO_MODE=true`** — on startup, if the database has no pets yet (fresh volume after migrations), demo seed runs automatically (append-only; never wipes). Use for PR preview hosts such as `petmon-pr.j0rsa.com` with OIDC (not `DEV_MODE`). Pair with a persistent `DATABASE_URL` volume so re-deploys keep data; wipe the volume to re-seed. The UI shows a sticky **Demo** banner when demo mode is on (`GET /api/v1/info` → `demo_mode: true`). PR CI builds with `VITE_APP_FLAVOR=pr` (grayscale icons + “Petmon PR” PWA name) and publishes `ghcr.io/j0rsa/petmon:pr-<number>-<run>-<sha>`, then triggers Renovate.
