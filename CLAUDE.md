...
**Feeding reminders** live on each `nutrition_schedules` row (`notify`), not `pet_settings`. Toggle is on Nutrition → Feeding, per schedule. Window `from`/`to` floor to 10-minute steps (UI floors on blur; the API floors on save) so a late time never wraps into the next day. A worker runs every 10 minutes; if today's intake is still below the **cumulative schedule projection** (`schedule_projection_at` — same midpoint stepping as the fluid chart's **schedule** line), it sends in-app + push to every subscriber: `Time to give some {food|liquid} to {pet}.` Deduped per schedule + date + window `from` (the latest started window when behind). Liquid schedules compare **total known fluid** (direct liquids + wet-food moisture at 0.77), the same metric as the cumulative chart's `total` series — not `direct_liquid_ml` alone. Food schedules compare grams (`wet_food_g + dry_food_g`).
